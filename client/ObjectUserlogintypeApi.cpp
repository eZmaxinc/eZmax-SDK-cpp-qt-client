...
ObjectUserlogintypeApi::ObjectUserlogintypeApi(const int timeOut)
    : _timeOut(timeOut),
      _manager(nullptr),
...
ObjectUserlogintypeApi::~ObjectUserlogintypeApi() {
}

void ObjectUserlogintypeApi::initializeServerConfigs() {
    //Default server
    ...
    _serverConfigs.insert("userlogintypeGetAutocompleteV2", defaultConf);
    _serverIndices.insert("userlogintypeGetAutocompleteV2", 0);
}
 ...
 */
int ObjectUserlogintypeApi::setDefaultServerValue(int serverIndex, const QString &operation, const QString &variable, const QString &value) {
 ...
 */
void ObjectUserlogintypeApi::setServerIndex(const QString &operation, int serverIndex) {
    if (_serverIndices.contains(operation) && serverIndex < _serverConfigs.find(operation).value().size()) {
        _serverIndices[operation] = serverIndex;
    }
}

void ObjectUserlogintypeApi::setServerIndex(int serverIndex) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
        setServerIndex(*keyIt, serverIndex);
    }
}

void ObjectUserlogintypeApi::setApiKey(const QString &apiKeyName, const QString &apiKey) {
    _apiKeys.insert(apiKeyName, apiKey);
}

void ObjectUserlogintypeApi::setBearerToken(const QString &token) {
    _bearerToken = token;
}

void ObjectUserlogintypeApi::setUsername(const QString &username) {
    _username = username;
}

void ObjectUserlogintypeApi::setPassword(const QString &password) {
    _password = password;
}


void ObjectUserlogintypeApi::setTimeOut(const int timeOut) {
    _timeOut = timeOut;
}

void ObjectUserlogintypeApi::setWorkingDirectory(const QString &path) {
    _workingDirectory = path;
}

void ObjectUserlogintypeApi::setNetworkAccessManager(QNetworkAccessManager* manager) {
    _manager = manager;
}
 ...
 */
int ObjectUserlogintypeApi::addServerConfiguration(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectUserlogintypeApi::setNewServerForAllOperations(const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectUserlogintypeApi::setNewServer(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    setServerIndex(operation, addServerConfiguration(operation, url, description, variables));
}

void ObjectUserlogintypeApi::addHeaders(const QString &key, const QString &value) {
    _defaultHeaders.insert(key, value);
}

void ObjectUserlogintypeApi::enableRequestCompression() {
    _isRequestCompressionEnabled = true;
}

void ObjectUserlogintypeApi::enableResponseCompression() {
...
QString ObjectUserlogintypeApi::getParamStyleSuffix(const QString &style) {
    if (style == "matrix") {
        return "=";
    } else if (style == "label") {
        return "";
...
QString ObjectUserlogintypeApi::getParamStyleDelimiter(const QString &style, const QString &name, bool isExplode) {
...
void ObjectUserlogintypeApi::userlogintypeGetAutocompleteV2Callback(HttpRequestWorker *worker) {
    ...
    Userlogintype_getAutocomplete_v2_Response output(QString(worker->response));
    worker->deleteLater();
...
void ObjectUserlogintypeApi::tokenAvailable() {
