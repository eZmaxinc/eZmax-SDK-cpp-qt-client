...
ObjectActivesessionApi::ObjectActivesessionApi(const int timeOut)
    : _timeOut(timeOut),
      _manager(nullptr),
...
ObjectActivesessionApi::~ObjectActivesessionApi() {
}

void ObjectActivesessionApi::initializeServerConfigs() {
    //Default server
 ...
 */
int ObjectActivesessionApi::setDefaultServerValue(int serverIndex, const QString &operation, const QString &variable, const QString &value) {
 ...
 */
void ObjectActivesessionApi::setServerIndex(const QString &operation, int serverIndex) {
    if (_serverIndices.contains(operation) && serverIndex < _serverConfigs.find(operation).value().size()) {
        _serverIndices[operation] = serverIndex;
    }
}

void ObjectActivesessionApi::setServerIndex(int serverIndex) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
        setServerIndex(*keyIt, serverIndex);
    }
}

void ObjectActivesessionApi::setApiKey(const QString &apiKeyName, const QString &apiKey) {
    _apiKeys.insert(apiKeyName, apiKey);
}

void ObjectActivesessionApi::setBearerToken(const QString &token) {
    _bearerToken = token;
}

void ObjectActivesessionApi::setUsername(const QString &username) {
    _username = username;
}

void ObjectActivesessionApi::setPassword(const QString &password) {
    _password = password;
}


void ObjectActivesessionApi::setTimeOut(const int timeOut) {
    _timeOut = timeOut;
}

void ObjectActivesessionApi::setWorkingDirectory(const QString &path) {
    _workingDirectory = path;
}

void ObjectActivesessionApi::setNetworkAccessManager(QNetworkAccessManager* manager) {
    _manager = manager;
}
 ...
 */
int ObjectActivesessionApi::addServerConfiguration(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectActivesessionApi::setNewServer(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    setServerIndex(operation, addServerConfiguration(operation, url, description, variables));
}

void ObjectActivesessionApi::addHeaders(const QString &key, const QString &value) {
    _defaultHeaders.insert(key, value);
}

void ObjectActivesessionApi::enableRequestCompression() {
    _isRequestCompressionEnabled = true;
}

void ObjectActivesessionApi::enableResponseCompression() {
...
QString ObjectActivesessionApi::getParamStyleSuffix(const QString &style) {
    if (style == "matrix") {
        return "=";
    } else if (style == "label") {
        return "";
...
QString ObjectActivesessionApi::getParamStyleDelimiter(const QString &style, const QString &name, bool isExplode) {
...
void ObjectActivesessionApi::activesessionGetCurrentV1Callback(HttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    Activesession_getCurrent_v1_Response output(QString(worker->response));
    worker->deleteLater();
...
void ObjectActivesessionApi::activesessionGetListV1(const ::Ezmaxapi::OptionalParam<QString> &e_order_by, const ::Ezmaxapi::OptionalParam<qint32> &i_row_max, const ::Ezmaxapi::OptionalParam<qint32> &i_row_offset, const ::Ezmaxapi::OptionalParam<Header_Accept_Language> &accept_language, const ::Ezmaxapi::OptionalParam<QString> &s_filter) {
...
void ObjectActivesessionApi::activesessionGetListV1Callback(HttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    Activesession_getList_v1_Response output(QString(worker->response));
    worker->deleteLater();
...
void ObjectActivesessionApi::tokenAvailable() {
