...
#include "ObjectFranchisebrokerApi.h"
...
ObjectFranchisebrokerApi::ObjectFranchisebrokerApi(const int timeOut)
    : _timeOut(timeOut),
      _manager(nullptr),
...
ObjectFranchisebrokerApi::~ObjectFranchisebrokerApi() {
}

void ObjectFranchisebrokerApi::initializeServerConfigs() {
    //Default server
 ...
 */
int ObjectFranchisebrokerApi::setDefaultServerValue(int serverIndex, const QString &operation, const QString &variable, const QString &value) {
 ...
 */
void ObjectFranchisebrokerApi::setServerIndex(const QString &operation, int serverIndex) {
    if (_serverIndices.contains(operation) && serverIndex < _serverConfigs.find(operation).value().size()) {
        _serverIndices[operation] = serverIndex;
    }
}

void ObjectFranchisebrokerApi::setServerIndex(int serverIndex) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
        setServerIndex(*keyIt, serverIndex);
    }
}

void ObjectFranchisebrokerApi::setApiKey(const QString &apiKeyName, const QString &apiKey) {
    _apiKeys.insert(apiKeyName, apiKey);
}

void ObjectFranchisebrokerApi::setBearerToken(const QString &token) {
    _bearerToken = token;
}

void ObjectFranchisebrokerApi::setUsername(const QString &username) {
    _username = username;
}

void ObjectFranchisebrokerApi::setPassword(const QString &password) {
    _password = password;
}


void ObjectFranchisebrokerApi::setTimeOut(const int timeOut) {
    _timeOut = timeOut;
}

void ObjectFranchisebrokerApi::setWorkingDirectory(const QString &path) {
    _workingDirectory = path;
}

void ObjectFranchisebrokerApi::setNetworkAccessManager(QNetworkAccessManager* manager) {
    _manager = manager;
}
 ...
 */
int ObjectFranchisebrokerApi::addServerConfiguration(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectFranchisebrokerApi::setNewServerForAllOperations(const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectFranchisebrokerApi::setNewServer(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    setServerIndex(operation, addServerConfiguration(operation, url, description, variables));
}

void ObjectFranchisebrokerApi::addHeaders(const QString &key, const QString &value) {
    _defaultHeaders.insert(key, value);
}

void ObjectFranchisebrokerApi::enableRequestCompression() {
    _isRequestCompressionEnabled = true;
}

void ObjectFranchisebrokerApi::enableResponseCompression() {
...
QString ObjectFranchisebrokerApi::getParamStyleSuffix(const QString &style) {
    if (style == "matrix") {
        return "=";
    } else if (style == "label") {
        return "";
...
QString ObjectFranchisebrokerApi::getParamStyleDelimiter(const QString &style, const QString &name, bool isExplode) {
...
void ObjectFranchisebrokerApi::franchisebrokerGetAutocompleteV2(const QString &s_selector, const ::Ezmaxapi::OptionalParam<QString> &e_filter_active, const ::Ezmaxapi::OptionalParam<QString> &s_query, const ::Ezmaxapi::OptionalParam<Header_Accept_Language> &accept_language) {
    ...
    connect(worker, &HttpRequestWorker::on_execution_finished, this, &ObjectFranchisebrokerApi::franchisebrokerGetAutocompleteV2Callback);
...
void ObjectFranchisebrokerApi::franchisebrokerGetAutocompleteV2Callback(HttpRequestWorker *worker) {
    ...
    Franchisebroker_getAutocomplete_v2_Response output(QString(worker->response));
    worker->deleteLater();
...
void ObjectFranchisebrokerApi::tokenAvailable() {
