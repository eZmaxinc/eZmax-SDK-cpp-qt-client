...
void ObjectWebhookApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("webhookSendWebhookV1", defaultConf);
    _serverIndices.insert("webhookSendWebhookV1", 0);
...
void ObjectWebhookApi::webhookSendWebhookV1(const Webhook_sendWebhook_v1_Request &webhook_send_webhook_v1_request) {
    QString fullPath = QString(_serverConfigs["webhookSendWebhookV1"][_serverIndices.value("webhookSendWebhookV1")].URL()+"/1/object/webhook/sendWebhook");
...

    {
        ...
        QByteArray output = webhook_send_webhook_v1_request.asJson().toUtf8();
        input.request_body.append(output);
    }
    for (auto keyValueIt = _defaultHeaders.keyValueBegin(); keyValueIt != _defaultHeaders.keyValueEnd(); keyValueIt++) {
        input.headers.insert(keyValueIt->first, keyValueIt->second);
...
void ObjectWebhookApi::webhookSendWebhookV1Callback(HttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
