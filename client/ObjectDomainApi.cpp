...
void ObjectDomainApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("domainGetAutocompleteV2", defaultConf);
    _serverIndices.insert("domainGetAutocompleteV2", 0);
...
void ObjectDomainApi::domainGetAutocompleteV2(const QString &s_selector, const ::Ezmaxapi::OptionalParam<QString> &e_filter_active, const ::Ezmaxapi::OptionalParam<QString> &s_query, const ::Ezmaxapi::OptionalParam<Header_Accept_Language> &accept_language) {
    QString fullPath = QString(_serverConfigs["domainGetAutocompleteV2"][_serverIndices.value("domainGetAutocompleteV2")].URL()+"/2/object/domain/getAutocomplete/{sSelector}");
    ...
    connect(worker, &HttpRequestWorker::on_execution_finished, this, &ObjectDomainApi::domainGetAutocompleteV2Callback);
    connect(this, &ObjectDomainApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this] {
        if (findChildren<HttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void ObjectDomainApi::domainGetAutocompleteV2Callback(HttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    Domain_getAutocomplete_v2_Response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT domainGetAutocompleteV2Signal(output);
        Q_EMIT domainGetAutocompleteV2SignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT domainGetAutocompleteV2SignalE(output, error_type, error_str);
        Q_EMIT domainGetAutocompleteV2SignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT domainGetAutocompleteV2SignalError(output, error_type, error_str);
        Q_EMIT domainGetAutocompleteV2SignalErrorFull(worker, error_type, error_str);
    }
}
