...
void ObjectInvoiceApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("invoiceImportIntoEDMV1", defaultConf);
    _serverIndices.insert("invoiceImportIntoEDMV1", 0);
}
...
void ObjectInvoiceApi::invoiceImportIntoEDMV1(const qint32 &pki_invoice_id, const Invoice_importIntoEDM_v1_Request &invoice_import_into_edm_v1_request) {
    QString fullPath = QString(_serverConfigs["invoiceImportIntoEDMV1"][_serverIndices.value("invoiceImportIntoEDMV1")].URL()+"/1/object/invoice/{pkiInvoiceID}/importIntoEDM");
...

    {
        ...
        QByteArray output = invoice_import_into_edm_v1_request.asJson().toUtf8();
    ...
    connect(worker, &HttpRequestWorker::on_execution_finished, this, &ObjectInvoiceApi::invoiceImportIntoEDMV1Callback);
    connect(this, &ObjectInvoiceApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this] {
        if (findChildren<HttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void ObjectInvoiceApi::invoiceImportIntoEDMV1Callback(HttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    Invoice_importIntoEDM_v1_Response output(QString(worker->response));
    worker->deleteLater();

    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT invoiceImportIntoEDMV1Signal(output);
        Q_EMIT invoiceImportIntoEDMV1SignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT invoiceImportIntoEDMV1SignalE(output, error_type, error_str);
        Q_EMIT invoiceImportIntoEDMV1SignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT invoiceImportIntoEDMV1SignalError(output, error_type, error_str);
        Q_EMIT invoiceImportIntoEDMV1SignalErrorFull(worker, error_type, error_str);
    }
}
