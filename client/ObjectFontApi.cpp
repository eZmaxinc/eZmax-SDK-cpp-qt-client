...
ObjectFontApi::ObjectFontApi(const int timeOut)
    : _timeOut(timeOut),
      _manager(nullptr),
...
ObjectFontApi::~ObjectFontApi() {
}

void ObjectFontApi::initializeServerConfigs() {
    //Default server
    ...
    _serverConfigs.insert("fontGetAutocompleteV2", defaultConf);
    _serverIndices.insert("fontGetAutocompleteV2", 0);
}
 ...
 */
int ObjectFontApi::setDefaultServerValue(int serverIndex, const QString &operation, const QString &variable, const QString &value) {
 ...
 */
void ObjectFontApi::setServerIndex(const QString &operation, int serverIndex) {
    if (_serverIndices.contains(operation) && serverIndex < _serverConfigs.find(operation).value().size()) {
        _serverIndices[operation] = serverIndex;
    }
}

void ObjectFontApi::setServerIndex(int serverIndex) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
        setServerIndex(*keyIt, serverIndex);
    }
}

void ObjectFontApi::setApiKey(const QString &apiKeyName, const QString &apiKey) {
    _apiKeys.insert(apiKeyName, apiKey);
}

void ObjectFontApi::setBearerToken(const QString &token) {
    _bearerToken = token;
}

void ObjectFontApi::setUsername(const QString &username) {
    _username = username;
}

void ObjectFontApi::setPassword(const QString &password) {
    _password = password;
}


void ObjectFontApi::setTimeOut(const int timeOut) {
    _timeOut = timeOut;
}

void ObjectFontApi::setWorkingDirectory(const QString &path) {
    _workingDirectory = path;
}

void ObjectFontApi::setNetworkAccessManager(QNetworkAccessManager* manager) {
    _manager = manager;
}
 ...
 */
int ObjectFontApi::addServerConfiguration(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectFontApi::setNewServerForAllOperations(const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    for (auto keyIt = _serverIndices.keyBegin(); keyIt != _serverIndices.keyEnd(); keyIt++) {
 ...
 * @param variables A map between a variable name and its value. The value is used for substitution in the server's URL template.
 */
void ObjectFontApi::setNewServer(const QString &operation, const QUrl &url, const QString &description, const QMap<QString, ServerVariable> &variables) {
    setServerIndex(operation, addServerConfiguration(operation, url, description, variables));
}

void ObjectFontApi::addHeaders(const QString &key, const QString &value) {
    _defaultHeaders.insert(key, value);
}

void ObjectFontApi::enableRequestCompression() {
    _isRequestCompressionEnabled = true;
}

void ObjectFontApi::enableResponseCompression() {
...
QString ObjectFontApi::getParamStyleSuffix(const QString &style) {
    if (style == "matrix") {
        return "=";
    } else if (style == "label") {
        return "";
...
QString ObjectFontApi::getParamStyleDelimiter(const QString &style, const QString &name, bool isExplode) {
...
void ObjectFontApi::fontGetAutocompleteV2(const QString &s_selector, const ::Ezmaxapi::OptionalParam<QString> &e_filter_active, const ::Ezmaxapi::OptionalParam<QString> &s_query, const ::Ezmaxapi::OptionalParam<Header_Accept_Language> &accept_language) {
    ...
    connect(worker, &HttpRequestWorker::on_execution_finished, this, &ObjectFontApi::fontGetAutocompleteV2Callback);
...
void ObjectFontApi::fontGetAutocompleteV2Callback(HttpRequestWorker *worker) {
    ...
    Font_getAutocomplete_v2_Response output(QString(worker->response));
    worker->deleteLater();
...
void ObjectFontApi::tokenAvailable() {
